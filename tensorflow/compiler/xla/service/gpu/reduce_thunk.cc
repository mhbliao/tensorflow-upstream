...
Status ReduceThunk::ExecuteOnStream(const BufferAllocations& buffer_allocations,
                                    se::Stream* stream,
                                    HloExecutionProfiler* profiler) {
  se::DeviceMemoryBase input_data =
      buffer_allocations.GetDeviceAddress(reduce_input_);
  se::DeviceMemoryBase reference_output_data =
      buffer_allocations.GetDeviceAddress(reduce_output_);
  se::DeviceMemoryBase output_data =
      buffer_allocations.GetDeviceAddress(output_);
  auto op_profiler = profiler->MakeScopedInstructionProfiler(hlo_instruction());
  VLOG(3) << "[ReduceThunk] Launch reduction for HLO: " << hlo_->ToString();
  stream->ThenReduce(input_data, &output_data, init_value_, reduce_dimension_);
  return Status::OK();
}
